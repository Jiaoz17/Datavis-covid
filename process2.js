...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function createCovidVisualization(data) {
    ...
    // Scale for circle radius - Use a minimum size to ensure visibility
    const rScale = d3.scaleSqrt()
      .domain([0, 10000000])  // Maximum represented value
      .range([3, 10000000 * confirmedScaleFactor / 2]);  // Minimum radius of 3px, max based on axis scale
      
    // Clamp the radius to a minimum and maximum value
    function getClampedRadius(value) {
      // Ensure a minimum visible size even for small values
      return Math.max(Math.min(rScale(value), 50), 5);  // Between 5px and 50px
      
    // Color scales for the circles - using more vibrant colors
    const colorScaleConfirmed = d3.scaleLinear()
      .domain([0, d3.max(monthlyArray, d => d.monthly_new_confirmed)])
      .range(["#e9d5ff", "#c026d3"]); // Light pink to vibrant purple
  
    const colorScaleDeceased = d3.scaleLinear()  
      .domain([0, d3.max(monthlyArray, d => d.monthly_new_deceased)])
      .range(["#dbeafe", "#2563eb"]); // Light blue to vibrant blue
    ...
    // Add circles for monthly confirmed cases with enhanced visibility
    svg.selectAll(".confirmed-circle")
      ...
      .attr("opacity", 0.9)  // Slightly more opaque
      .append("title")
      .text(d => `${d.month}: ${d3.format(",.0f")(d.monthly_new_confirmed)} new cases`);
  
    // Add circles for monthly deceased cases with enhanced visibility
